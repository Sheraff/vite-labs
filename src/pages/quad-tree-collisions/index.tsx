...
function start(
	...
	const tree = new TreeNode<Ball>(0, 0, ctx.canvas.width, ctx.canvas.height, 0)
...
class TreeNode<T extends { x: number, y: number } = { x: number, y: number }> {
	static MAX_DEPTH = 6
	...
	constructor(
		...
		if (depth < TreeNode.MAX_DEPTH) {
			...
			this.children = [
				new TreeNode(x, y, halfWidth, halfHeight, depth + 1, this),
				new TreeNode(x + halfWidth, y, halfWidth, halfHeight, depth + 1, this),
				new TreeNode(x, y + halfHeight, halfWidth, halfHeight, depth + 1, this),
				new TreeNode(x + halfWidth, y + halfHeight, halfWidth, halfHeight, depth + 1, this),
			]
		}
	}
	...
	query(x: number, y: number, radius: number): Set<T> {
		...
		if (this.children) {
			...
			for (const child of this.children) {
				const objects = child.query(x, y, radius)
				objects.forEach(obj => result.add(obj))
			}
			return result
		} else {
			return this.objects
