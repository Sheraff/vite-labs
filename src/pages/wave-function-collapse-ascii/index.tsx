...
function collapseNeighbors(array: Cell[], cell: Cell, backtrack: Backtrack, stack: Cell[] = []) {
	for (let i = 0; i < NEIGHBORS.length; i++) {
		...
		if (x < 0 || x >= GRID_SIZE[0] || y < 0 || y >= GRID_SIZE[1]) continue
		const neighbor = getCellFromXY(array, x, y)
		if (neighbor.tile) continue
		const possibilities = cell.tile ? [cell.tile.is[i]] : cell.accepts[i]
		const mirrorIndex = (i + 2) % 4
		const compatibleOptions = neighbor.accepts[mirrorIndex].filter((option) => possibilities.includes(option))
		if (compatibleOptions.length === 0) throw new Error("Invalid neighbor")
		const entropyDifference = neighbor.accepts[mirrorIndex].length - compatibleOptions.length
		if (entropyDifference !== 0) stack.push(neighbor)
...
function assignSuitableOption(array: Cell[], cell: Cell, backtrack: Backtrack) {
	const options = TILES.filter((option) =>
		cell.accepts.every((directionalOption, i) => directionalOption.includes(option.is[i])),
	)

	if (options.length === 0) throw new Error("No suitable options")
