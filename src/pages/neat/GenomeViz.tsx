...
export function prepareViz(genome: Type, ctx: CanvasRenderingContext2D) {
	...
	const accumulators = new Map<number, [min: number, max: number]>()
	for (const node of nodes.values()) {
		accumulators.set(node.index, [Infinity, -Infinity])
	}
	function drawNodes(memory: Float32Array) {
		for (const node of nodes.values()) {
	...
	function draw(memory: Float32Array) {
