...
export default function Neat() {
	...
	const [food] = useState<Food>(() =>
		Array.from(
			{ length: FOOD_COUNT },
			() => [Math.random() * (side - 100) + 50, Math.random() * (side - 100) + 50] as const,
		),
	)
	...
	useEffect(() => {
		...
		drawnBatch(entities, { ctx: context, side, controller, food, controls: controlsPlay.current }, () =>
				...
				<fieldset>
					...
					{autoplay && (
						<>
							...
							/>
							<label htmlFor="range">
								Playing generation {selected.toString().padStart(2, "0")} of{" "}
								{displayGeneration.toString().padStart(2, "0")}
							</label>
							<button
								type="button"
								disabled={!autoplay}
...
function postMessage<T extends Incoming["type"]>(
	worker: Worker,
	type: T,
	data: Extract<Incoming, { type: T }>["data"],
) {
