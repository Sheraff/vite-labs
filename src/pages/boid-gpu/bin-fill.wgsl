// 2D spatial binning (z is checked separately in update shader)
struct Config {
	binWidth: f32,
	binHeight: f32,
	widthDivisions: u32,
...
@group(2) @binding(0) var<storage, read> particlePositions: array<vec4f>;
...
fn size(
	...
	let position = particlePositions[i].xy;
	let index = getBinIndex(position);
