...
const RadioStarsInput = ({ name, label }: { name: string; label: string }) => {
	...
	return (
		<fieldset className={styles.radio} style={{ "--svg-bg-image": img } as CSSProperties}>
			<legend>{label}</legend>
...
const RangeStarsInput = ({ name, label }: { name: string; label: string }) => {
	...
	return (
		...
		>
			...
			<input id="range" type="range" name={name} min={min} max={max} step={step} onChange={onChange} value={value} />
		</div>
	)
}
